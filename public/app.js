// public/app.js — Étude 28 rubriques (LLM-free) + api.bible
// - Génère les 28 points via /api/study-28 (moteur interne, sans OpenAI).
// - Injection contextuelle de versets via /api/verse.
// - Liens cliquables BibleGateway, rendu "mode enrichi", autosave, etc.
...
(function () {
  ...
  function findBibleRefs(text){
    const out = []; const raw = String(text || ""); let m;
    while ((m = refRe.exec(raw))) {
      const book = m[1], chap = m[2], verseOrRange = m[3] || "", chapEnd = m[4] || "";
      out.push({ label: m[0], book, chap: +chap, verseOrRange, chapEnd });
    }
    return out;
  }
  ...
  async function fetchJSON(url, tries = 3){
    ...
    for (let k=0;k<tries;k++){
      try{
        ...
        return await r.json();
      }catch(e){ lastErr = e; await wait(180*(k+1)); }
    }
    throw lastErr;
  }
  ...
  async function getStudy() {
    ...
    const chapter = String(chapterSelect?.value || "1");
    const verse = verseSelect?.value || "";

    const sp = new URLSearchParams({ book, chapter, translation: ver, mode: "full" });
    if (verse) sp.set("verse", verse);

    const j = await fetchJSON(`/api/study-28?${sp.toString()}`);
    if (!j || j.ok === false) throw new Error(j?.error || "Réponse invalide");
    const data = j.data || {};
    const sections = Array.isArray(data.sections) ? data.sections : [];
    window.__lastChatSource = "study-28/api.bible"; // pour log debug

    // normalise vers format attendu {id,title,content}
    const norm = sections.map((s, i) => ({
      id: s.index || (i+1),
      title: s.title || `Section ${i+1}`,
      content: String(s.content || "")
    }));
    return { from: "study-28", data: { reference: data.meta?.reference || `${book} ${chapter}`, sections: norm } };
  }
  ...
  async function postJSON(url, payload, tries = 2){
    ...
    for (let i=0;i<tries;i++){
      try{
        ...
        if (!r.ok) throw new Error(`HTTP ${r.status}`);
        return await r.json();
      }catch(e){ last=e; await wait(180*(i+1)); }
    }
    throw last;
  }
  ...
  async function fetchVerseOnce({ book, chapter, verse, version }) {
    try{
      const j = await postJSON("/api/verse", { book, chapter, verse, version });
      if (j && j.ok && j.text) return j;
    }catch(e){ dlog(`[VERSE] ${e?.message||e}`); }
    return null;
  }
  ...
  function findBibleRefs(text){
    const out = []; const raw = String(text || ""); let m;
    while ((m = refRe.exec(raw))) {
      out.push({ label:m[0], book:m[1], chap:+m[2], verseOrRange:m[3]||"", chapEnd:m[4]||"" });
    }
    return out;
  }
  ...
  async function injectVersesIntoSection(idx, policy) {
    ...
    const refs = findBibleRefs(content);
    const unique = [];
    const seen = new Set();
    for (const r of refs) {
      if (!r.verseOrRange || /[–-]/.test(r.verseOrRange)) continue; // seulement verset simple
    ...
    selected = selected.slice(0, Math.max(1, policy.max || 3));

    const version = (versionSelect && versionSelect.value) || "LSG";
    const outBlocks = [];
    for (const it of selected) {
      const v = await fetchVerseOnce({ ...it, version });
      if (v) outBlocks.push(`— **Texte biblique** (${v.reference}, ${v.version || "LSG"})\n${v.text}`);
    }
  ...
  async function generateStudy() {
    ...
    try {
      ...
      // si un verset précis est sélectionné → injection en 0
      try {
        const hasVerse = verseSelect && verseSelect.value && Number(verseSelect.value) > 0;
        if (hasVerse) {
          const j = await postJSON("/api/verse", {
      ...
      const titleToIndex = {}; FIXED_POINTS.forEach((p, i) => titleToIndex[p.t] = i);
      const injectPlan = [
  ...
  verseSelect.addEventListener("change", () => { /* optionnel */ });
