...
// → Branche DIRECTEMENT sur /api/study-28 (LLM-FREE + api.bible)
// - MetaInfo : "Point X / 28"
...
// - Pastilles de santé: health, study-28(selftest), ping
...
(function () {
  ...
  // ---------- mapping version -> bibleId ----------
  // Laisse vide pour "auto" (le backend choisira la première FR).
  const BIBLE_IDS = {
    JND: "a93a92589195411f-01"   // Darby api.bible
    // LSG: "", // si tu ajoutes une autre bible avec son ID api.bible, mets-la ici
  };
  ...
  function renderViewFromArea(){
    ...
    html = boldBibleRefs(html);        // 3) force le gras sur refs
    html = mdLite(html);               // 4) **bold** / *italique*
  ...
  // ---------- getStudy (→ /api/study-28) ----------
  async function getStudy() {
    const translation = versionSelect?.value || "JND";
    const book  = bookSelect?.value || "Genèse";
    const chapter = String(chapterSelect?.value || "1");
    const verse = (verseSelect && verseSelect.value) || ""; // vide => chapitre entier

    // auto bibleId selon la version (JND => Darby)
    let bibleId = "";
    if (BIBLE_IDS[translation]) bibleId = BIBLE_IDS[translation];

    const sp = new URLSearchParams({ book, chapter, translation, trace: "1" });
    if (verse) sp.set("verse", verse);
    if (bibleId) sp.set("bibleId", bibleId);

    const r = await fetch("/api/study-28?" + sp.toString(), { headers: { "accept":"application/json" }});
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
    const j = await r.json();
    if (!j || j.ok === false) throw new Error(j?.error || "Réponse JSON invalide");

    return { from: "study-28", data: j.data };
  }
  ...
  async function generateStudy() {
    ...
    try {
      ...
      secs.forEach((s) => {
        const i = (s.index | 0) - 1; // index (1-based) → 0-based
        if (i >= 0 && i < N) {
      ...
      HOOK('be:study-generated', {
        reference: data.meta?.reference || ref,
        filled: Object.keys(notes).filter(k => (notes[k] || "").trim()).map(k => +k),
        notes
      });

      setProgress(100); setTimeout(() => setProgress(0), 300);
      dlog(`[GEN] source=study-28 sections=${secs.length} filled=${Object.keys(notes).length} → étude générée`);
    } catch (e) {
      console.error(e);
  ...
  // debug panel (health + study-28 selftest + ping)
  dbtn && dbtn.addEventListener("click", () => {
    ...
    if (!open) {
      ...
      (async () => {
        ...
        try { const r2 = await fetch("/api/study-28?selftest=1", {cache:"no-store"}); setMini(dotChat, r2.ok); dlog(`study-28(selftest) → ${r2.status}`); } catch { setMini(dotChat, false); }
