...
(function () {
  ...
  const dlog = (msg) => {
    ...
    btnDbg.textContent = "Fermer Debug";
  ...
  function scoreFor(section, target) {
    ...
    let s = 0;
    target.k.forEach((k) => {
      if (t.includes(k)) s += 3; // titre pèse plus
      if (b.includes(k)) s += 1;
    });
  ...
  async function generateStudy() {
    ...
    if (!rawSections.length) {
      dlog(`[API OK mais sans sections]`);
    ...
    // si mapping insuffisant, compléter positionnel
  ...
  // Valider → BibleGateway (pas d'appel API)
  ...
  // Auto-génération si pas de texte dans la barre de recherche
