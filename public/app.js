...
(function () {
  ...
  // ---------- WHOAMI (bible par défaut côté serveur) ----------
  window.__DEFAULT_BIBLE_ID = "";
  fetch("/api/utils?action=whoami", { cache: "no-store" })
    .then(r => r.json())
    .then(j => { if (j && j.ok && j.defaultBibleId) window.__DEFAULT_BIBLE_ID = j.defaultBibleId; })
    .catch(()=>{});
  ...
  if (enrichToggle){
    enrichToggle.addEventListener("change",()=>{
      if (enrichToggle.checked){ noteArea.style.display="none"; noteView.style.display="block"; renderViewFromArea(); }
      else { noteView.style.display="none"; noteArea.style.display="block"; }
    });
  }
  ...
  // Lecture/validation: ouvre BibleGateway (visuel externe pratique)
  readBtn.addEventListener("click",()=>{
    const b=bookSelect.value, c=chapterSelect.value, v=verseSelect.value, ver=versionSelect.value||"LSG";
    window.open(`https://www.biblegateway.com/passage/?search=${encodeURIComponent(`${b} ${c}${v?':'+v:''}`)}&version=${encodeURIComponent(ver)}`,"_blank","noopener");
  });
  validateBtn.addEventListener("click",()=>{ updateLinksPanel(); });

  // (facultatif) autogénération quand la sélection change
  ...
  bookSelect.addEventListener("change", autoGenerate);
