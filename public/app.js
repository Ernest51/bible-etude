/* app.js — Palette globale + Bouton "Lire la Bible" fiable
   (Conserve : 66 livres+chapitres, densité 500/1500/2500, Valider, API generate-study, 28 rubriques) */
...
(function () {
  ...
  function dbgInit(){
    ...
    el.id='debugMini';
    el.style.cssText='position:fixed;bottom:8px;left:8px;right:8px;max-height:28vh;overflow:auto;background:#0b1020;color:#cbd5e1;padding:8px 10px;border-radius:8px;font-size:12px;z-index:9999;opacity:.92';
    el.textContent='Debug prêt.\n';
    document.body.appendChild(el);
  }
  function log(...a){ try{ const d=$('#debugMini'); if(d){ d.textContent+=a.map(x=>typeof x==='string'?x:JSON.stringify(x)).join(' ')+'\n'; d.scrollTop=d.scrollHeight; } }catch{} }
  ...
  function init(){
    ...
    restoreTheme();           // ← applique thème global dès le chargement
    ...
    log('Init OK —', state.book, state.chapter, 'dens:', state.density, 'theme:', getTheme());
  }
  ...
  function wireEvents(){
    ...
    // Bouton BibleGateway — version robuste (toujours un vrai click utilisateur)
    readBtn && readBtn.addEventListener('click', openBibleGatewayFromSelectors);
  ...
  function applySearch(){
    ...
    if (m){
      ...
      if (found){
        ...
        bookSelect&&(bookSelect.value=state.book); fillChapters(); chapterSelect&&(chapterSelect.value=String(state.chapter)); fillVerses(); if(vers) verseSelect&&(verseSelect.value=String(state.verse));
  ...
  function findBook(norm){ for(const b of ORDER_66){ const n=normalizeBook(b); if(n===norm||n.startsWith(norm)||norm.startsWith(n)) return b; } return null; }
...
À générer…`;
  ...
  async function onGenerate(){
    ...
    try{
      ...
      log('Génération OK rubriques:', sections.length);
    }catch(err){
...
Contenu provisoire (gabarit).`);
  ...
  // ---------- Palette : applique le thème à TOUT le site ----------
  const THEMES = ['cyan','violet','vert','rouge','mauve','indigo','ambre','slate'];

  function setTheme(theme){
    // Pose l’attribut sur <html> ET <body> pour couvrir tout le CSS
  ...
  // ---------- Bouton Lire la Bible (BibleGateway fiable) ----------
  function openBibleGatewayFromSelectors(){
    const book = state.book;
    const chap = state.chapter;
    const vers = (verseSelect && verseSelect.value && Number(verseSelect.value)>0) ? Number(verseSelect.value) : null;
    const version = state.version;
    const url = bibleGatewayURL(book, chap, vers, version);
    // Ouverture via un click utilisateur : _blank + noreferrer
    window.open(url, '_blank', 'noopener,noreferrer');
  }
  function bibleGatewayURL(book, chapter, verse, version){
  ...
  function restoreLast(){ try{ const raw=localStorage.getItem(STORAGE_LAST); if(!raw) return; const j=JSON.parse(raw); if(j&&CHAPTERS_66[j.book]){ state.book=j.book; state.chapter=Math.max(1,Math.min(CHAPTERS_66[j.book], parseInt(j.chapter,10)||1)); state.verse=parseInt(j.verse,10)||1; state.version=j.version||'LSG'; if(DENSITY_CHOICES.includes(j.density)) state.density=j.density; } }catch{} }

  function clamp(v,a,b){return Math.max(a,Math.min(b,v));}
  function onReset(){ if(!confirm('Tout vider et repasser en gabarit ?')) return; state.sectionsByN.clear(); for(let i=0;i<=28;i++) state.leds.set(i,'warn'); renderPointsList(); renderSection(state.currentIdx); }
})();
