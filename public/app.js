...
(function () {
  ...
  // ---------- état ----------
  let current=0, notes={}, autosaveTimer=null, autoTimer=null;
  ...
  function applySelection(sel){
    if(!sel) return; const idx=BOOKS.findIndex(([n])=>n===sel.book); if(idx>=0) bookSelect.selectedIndex=idx;
    renderChapters(); const chMax=bookSelect.selectedOptions[0]?+bookSelect.selectedOptions[0].dataset.ch:1; const chap=Math.max(1,Math.min(chMax, sel.chap||1)); chapterSelect.value=String(chap);
    renderVerses(sel.book==="Psaumes"?200:60); if(sel.vers) verseSelect.value=String(sel.vers); updateReadLink();
  }
  ...
  async function fetchChat(ref){
    ...
    try{ const r=await fetch(url); const j=await r.json().catch(()=>({})); if(r.ok && (j?.ok||j?.data)) return {ok:true,payload:j,status:r.status}; }catch(_){}
  ...
  // ---------- matching (scoring) ----------
  const PROFILE = [
    {i:0,k:["priere","ouverture","saint esprit","amen"]},
    {i:1,k:["canon","testament","corpus","livre"]},
    {i:2,k:["questions","precedent","retour","revisions","reponses"]},
    {i:3,k:["titre","resume","synthese","theme central"]},
    {i:4,k:["contexte","historique","epoque","geographie","culture"]},
    {i:5,k:["structure","plan","sequences","composition"]},
    {i:6,k:["genre","litteraire","poetique","narratif","proph"]},
    {i:7,k:["auteur","genealogie","biographie"]},
    {i:8,k:["verset","cle","central","doctrinal"]},
    {i:9,k:["exegese","exegetique","mot a mot","grec","hebreu"]},
    {i:10,k:["lexique","lexical","mots cles","termes"]},
    ...
    {i:16,k:["appui doctrinal","appui","renforce"]},
  ...
  function scoreFor(section,target){
    const t=norm(section.title||''), b=norm(section.body||''); let s=0;
    target.k.forEach(k=>{ if(t.includes(k)) s+=3; if(b.includes(k)) s+=1; });
    if(t && target.k.some(k=>t.startsWith(k))) s+=2; return s;
  }
  function assignSections(sections){
    const M=sections.map(sec=>PROFILE.map(p=>scoreFor(sec,p)));
    const taken=new Set(); const assigned=Array(sections.length).fill(-1);
    sections.forEach((sec,si)=>{ let best=-1,bestS=0; PROFILE.forEach((p,ri)=>{ if(taken.has(p.i)) return; const sc=M[si][ri]; if(sc>bestS){bestS=sc; best=p.i;} }); if(bestS>0){assigned[si]=best; taken.add(best);} });
    return assigned;
  }
  ...
  async function generateStudy(){
    ...
    const secs = rawSections.slice(0,40).map(s=>({ title:s.title||s.titre||'', body:String(s.content??s.description??s.text??'') })).filter(s=>s.title.trim()||s.body.trim());

    // mapping
    const assigned = assignSections(secs);
    const usedRub = new Set(assigned.filter(i=>i>=0));
    let mapped = usedRub.size;
    if(mapped < Math.min(12, secs.length)){ // secours positionnel
      for(let i=0;i<Math.min(N,secs.length);i++){ if(!usedRub.has(i) && !(assigned[i]>=0)){ assigned[i]=i; usedRub.add(i); mapped++; } }
    }

    // remplir notes (sauf prières locales)
    notes = {};
    for(let si=0; si<secs.length; si++){
      const ri=assigned[si];
      if(ri>=0 && ri<N && ri!==0 && ri!==27){ // pas les prières
        const body = secs[si].body.trim();
        if(body && !notes[ri]) notes[ri]=body;
      }
    }

    // ----- fallbacks & garanties -----
    // 0 & 27 : prières toujours locales
    notes[0]  = defaultPrayerOpen(data.reference || ref);
    notes[27] = defaultPrayerClose(data.reference || ref);

    // 1 : canon/testament auto
    if(!notes[1]){
      const idx = bookSelect.selectedIndex;
      const testament = idx < NT_START_INDEX ? "Ancien Testament" : "Nouveau Testament";
      notes[1] = `Le livre de ${bookSelect.value} appartient à l’${testament}.`;
    }

    // 2 : questions précédent (placeholder explicite)
    if(!notes[2]){
    ...
    // 8 : verset-clé garanti
    notes[8] = ensureKeyVerse(notes[8], data.reference || ref);

    dlog(`[GEN] sections=${secs.length}, mapped≈${mapped}, filled=${Object.keys(notes).length}`);
