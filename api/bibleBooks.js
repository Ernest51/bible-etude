...
/**
 * Retourne la liste des livres d’une Bible donnée
 * Usage: /api/bibleBooks?bibleId=a93a92589195411f-01
 */
export default async function handler(req) {
  try {
    ...
    if (!bibleId) {
      return new Response(
        JSON.stringify({ ok: false, error: "Paramètre 'bibleId' requis." }),
        { status: 400, headers: { "content-type": "application/json" } }
      );
    }

    const url = `${BASE_URL}/bibles/${bibleId}/books`;
