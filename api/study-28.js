...
// Vercel-compatible (runtime: "nodejs"), erreurs explicites
...
async function resolveBookId(bibleId, bookName) {
  ...
  if (!hit) { const e = new Error(`Book not found: ${bookName}`); e.status = 400; throw e; }
  return hit.id;
}
...
export default async function handler(req, res) {
  try {
    ...
    if (dry || !KEY) {
      // Mode hors-ligne (ou clé absente) → ne tente rien vers api.bible
      const sections = mode === "mini"
        ? makeMiniSections(referenceDry, "Exemple de texte.")
        : makeFullSections(referenceDry, "Exemple de texte.");
      return send(req, res, KEY ? 200 : 500, {
        ok: !!KEY,
        warn: KEY ? undefined : "API_BIBLE_KEY missing",
        data: { meta:{ book, chapter, verse, translation, reference: referenceDry, osis:"" }, sections }
      });
    }
    ...
    return send(req, res, status, { ok:false, error: String(e.message || e) });
  }
}
