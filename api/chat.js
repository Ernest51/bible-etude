...
function refString(book, chapter, verse){ const ch = clamp(parseInt(chapter,10),1,150); return verse ? `${cap(book)} ${ch}:${verse}` : `${cap(book)} ${ch}`; }
...
function dedupeAgainst(text, seenSet){
  ...
  const kept=[]; for(const p of parts){ const k=p.trim().replace(/\s+/g,' '); if(!k) continue; const sig=k.toLowerCase(); if(seenSet.has(sig)) continue; seenSet.add(sig); kept.push(p); }
  return kept.join(' ');
}
...
async function fetchVerseText(req, { book, chapter, verse, version="LSG" }, timeoutMs=3500){
  ...
  try{
    const r = await fetch(`${baseUrl(req)}/api/verse`, {
      ...
      body: JSON.stringify({ book, chapter, verse, version }), signal: ctrl.signal
    });
...
function crossRefsFor(book, chapter, testament, genre){
  ...
  if ((B==="genèse"||B==="genese") && Number(chapter)===1)
    arr.push("Psaumes 33:6","Jean 1:1-3","Hébreux 11:3","Colossiens 1:16","Psaumes 104:24","Exode 34:6-7","Josué 1:8-9");
...
function refsListHtml(refs, version="LSG", title="Références à consulter"){
  ...
  return `<h4>${esc(title)}</h4><ul class="ref-list">${lis}</ul>`;
}
...
const GLOSS = {
  ...
  "Typologie":"Lecture des figures/personnes/événements annonçant et culminant dans le Christ.",
  "Eschatologie":"Doctrine des choses dernières : espérance, jugement, nouvelle création.",
  "Sapiential":"Relatif à la sagesse biblique qui forme caractère, discernement et conduite."
};
function buildGlossarySection(){
  const items = Object.keys(GLOSS).sort().map(k=>`<li><strong>${esc(k)}</strong> — ${esc(GLOSS[k])}</li>`).join("");
  return `<h3>Glossaire</h3><ul>${items}</ul>`;
}
function injectGlossaryIfTermsAppear(html){
  const found=[]; for(const k of Object.keys(GLOSS)){ const re=new RegExp(`\\b${k}\\b`,'i'); if(re.test(html)) found.push(k); }
  if (!found.length) return html;
  const items = found.map(k=>`<li><strong>${esc(k)}</strong> — ${esc(GLOSS[k])}</li>`).join("");
  return html + `<details><summary><strong>Glossaire (termes rencontrés)</strong></summary><ul>${items}</ul></details>`;
}
...
function chooseKeyVerse({ book, chapter }){ const B=(book||"").toLowerCase(), ch=Number(chapter||1); if((B==="genèse"||B==="genese")&&ch===1) return 1; if(B==="jean"&&ch===1) return 14; if(B==="psaumes") return 1; return 1; }
function chooseMemoryVerse({ book, chapter, keyVerse }){ const B=(book||"").toLowerCase(); if(B==="psaumes") return 2; return keyVerse!==2?2:3; }
function chooseRetainVerses({ book, chapter, keyVerse }){ const picks=new Set([1,keyVerse,2,5]); return Array.from(picks).filter(v=>v>0).slice(0,5).sort((a,b)=>a-b); }
...
async function enrichVerses(req, sections, { book, chapter, version }){
  ...
  const items=[]; for(const v of list27){ const t = await fetchVerseText(req, { book, chapter, verse:v, version }); if(t) items.push({ v, t }); }
...
- Longueur cible ~2500 caractères (±10%), sans remplissage.
...
function fallbackSection({ sectionId, book, chapter, verse, version, testament, genre, motifs, versesText, seed }){
  ...
  if (sectionId === 6) {
    const bullets = {
      narratif: `<ul class="ref-list"><li>Ouverture : situation et appel (${aRef("Exode 3:1-10", version)})</li><li>Déroulement : actions et réponses (${aRef("Marc 1:16-20", version)})</li><li>Clôture : signe, bénédiction ou jugement (${aRef("Josué 24:14-24", version)})</li></ul>`,
      poétique: `<ul class="ref-list"><li>Parallélismes et images (${aRef("Psaumes 19:8-11", version)})</li><li>Progression affective/sapientiale (${aRef("Proverbes 3:5-7", version)})</li><li>Clôture : louange, confiance ou silence (${aRef("Psaumes 62:2", version)})</li></ul>`,
      prophétique:`<ul class="ref-list"><li>Diagnostic spirituel (${aRef("Ésaïe 1:2-4", version)})</li><li>Oracle : jugement/promesse (${aRef("Jérémie 31:31-34", version)})</li><li>Appel à revenir (${aRef("Joël 2:12-13", version)})</li></ul>`,
      épistolaire:`<ul class="ref-list"><li>Indicatifs de l’Évangile (${aRef("Romains 3:21-26", version)})</li><li>Impératifs de la vie nouvelle (${aRef("Colossiens 3:12-17", version)})</li><li>Applications communautaires (${aRef("Éphésiens 4:1-3", version)})</li></ul>`
    }[genre];
    return sanitizeHtmlAll(para(`Structure littéraire selon le genre.`) + bullets + vq(0) + refsListHtml(listRefs, version));
  }
  ...
  const blocks = {
    ...
    5:  [ para(`Repères historiques/culturels/théologiques pour situer l’intention du passage.`), vq(0), refsListHtml(listRefs, version) ],
    ...
    11: [ para(`<strong>Analyse lexicale.</strong> 1–3 termes décisifs (ex.: alliance, justice, sagesse, foi, esprit).`), vq(1), refsListHtml(listRefs, version), buildGlossarySection() ],
...
function openingFallback(book, chapter, verse, version){
  ...
  const html = `<p><strong>${head}</strong>, ${attr}, nous venons à toi devant <strong>${esc(ref)}</strong>. ${mid} ${end}</p>`;
  return sanitizeHtmlAll(linkifyAllVerses(html, version));
}
function closingFallback(book, chapter, verse, version){
  ...
  const html = `<p><strong>${head}</strong>, ${attr}, merci pour <strong>${esc(ref)}</strong>. ${mid} ${end}</p>`;
  return sanitizeHtmlAll(linkifyAllVerses(html, version));
}
      ...
      if (OPENAI_API_KEY){
        try{
          ...
          const prompt = {
            system: "Tu es un bibliste pédagogue. HTML autorisé: <p>, <em>, <strong>, <ul>, <li>, <blockquote>. Inclure 3–6 références bibliques explicites (style « Livre 1:1 ») différentes.",
            user: `
Réponds aux 5 questions de révision pour ${reference} (${version}).
- Testament=${testament}, Genre=${genre}
- Utiliser ≥2 motifs: ${JSON.stringify(m)}
- Longueur cible ~2500 caractères. Évite les lieux-communs.
- Donne UNIQUEMENT le HTML demandé.

Extraits (optionnels):
${[sample.vk, sample.vm].filter(Boolean).map(v=>`- ${v.ref}: “${v.text}”`).join('\n') || '—'}
`.trim()
          };
      ...
      sections[i3].content = sanitizeHtmlAll(linkifyAllVerses(Q + "\n" + A, version));
    }
    ...
    // 4) Renforcement 2,4,5,6,7,8,10–26 (20/27 déjà faits)
    ...
    for (const id of TARGET_IDS){
      ...
      const goalById = {
        ...
        11:"éclairer 1–3 termes clés + livrer un glossaire",
      ...
      if (!html || html.length < 500){
        html = fallbackSection({
          sectionId:id, book, chapter, verse, version, testament, genre, motifs, versesText, seed:simpleHash(`${reference}|${id}`)
        });
      }

      html = linkifyAllVerses(html, version);        // ← auto-liens pour TOUTES les refs
      html = html.replace(/<\/a>\s*(?=<a\b)/g,'</a> · '); // ← espacement visible
    ...
    // 5) Dernier filet de sécurité : linkify + nettoyage sur toutes sections
    for (const s of sections){
      s.content = sanitizeHtmlAll(linkifyAllVerses(s.content, version));
    }
