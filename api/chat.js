...
function parseRef(raw) {
  ...
  return { book: book.trim(), chapter: Number(chapter), verses: verses ? verses.replace(/\s+/g, '') : null, raw: s };
}

function buildMessages({ book, chapter, verses, raw, templateId }) {
  const system = `
Tu génères des études bibliques en **28 points**.
RÈGLES:
- Français uniquement.
- Strictement le passage demandé.
...
CONTRAINTES: 28 sections exactes; title ≤ 90c; content ≤ 700c; verses = tableau de références; aucun texte hors JSON.
`.trim();
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method !== 'GET' && req.method !== 'POST') {
      return send(res, 405, { ok: false, error: 'Méthode non autorisée (GET/POST)' });
    }
    ...
    const templateId = (typeof body?.templateId === 'string' && body.templateId) || getQuery(req).get('templateId') || 'v28-standard';
    ...
    if (!resp.ok) return send(res, 502, { ok: false, error: 'OpenAI API error', status: resp.status, body: tryJson(raw) });
    ...
    if (!Array.isArray(parsed.sections) || parsed.sections.length !== 28) {
      return send(res, 502, { ok: false, error: 'Le résultat ne contient pas 28 sections', got: Array.isArray(parsed.sections) ? parsed.sections.length : typeof parsed.sections });
    }

    return send(res, 200, { ok: true, data: { reference: parsed.reference ?? ref.raw, templateId, sections: parsed.sections } });
