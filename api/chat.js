// api/chat.js — JSON-mode “béton” : 28 rubriques garanties
// - GET ?q="Genèse 1" ou POST {book, chapter, version}; ?probe=1
// - Appel OpenAI (REST) avec response_format JSON -> s1..s28
// - Si erreur/format invalide: fallback “riche” (style Genèse 1)
...
// Fallback “riche” proche de ton modèle
function fallbackMarkdown(book,chapter){
  const AT = !["Matthieu","Marc","Luc","Jean","Actes","Romains","1 Corinthiens","2 Corinthiens","Galates","Éphésiens","Philippiens","Colossiens","1 Thessaloniciens","2 Thessaloniciens","1 Timothée","2 Timothée","Tite","Philémon","Hébreux","Jacques","1 Pierre","2 Pierre","1 Jean","2 Jean","3 Jean","Jude","Apocalypse"].includes(book);
  ...
  const obj={
    ...
    S5:`La Genèse, traditionnellement attribuée à Moïse, fonde l’identité d’Israël. ${book} ${chapter} s’inscrit contre les cosmogonies païennes : ici, un seul Dieu, personnel et souverain, crée par sa Parole.`,
    S6:`Jour 1 : Lumière/ténèbres • Jour 2 : Étendue • Jour 3 : Terre/végétation • Jour 4 : Astres • Jour 5 : Poissons/oiseaux • Jour 6 : Animaux/humain • Jour 7 : Repos (2:1-3).`,
    S7:`Narratif théologique (prose solennelle).`,
    S8:`Auteur : Moïse (tradition), rattaché aux patriarches.`,
    S9:`Genèse ${chapter}:27 — « Dieu créa l’homme à son image… » (LSG).`,
    S10:`bara’ (créer) réservé à Dieu ; tohu-bohu (v.2) : chaos ; tselem Elohim (image de Dieu) : dignité/vocation.`,
    S11:`tôv (bon), raqia (étendue), dominer/soumettre : gérance responsable.`,
    S12:`Jean 1:1-3 ; Col 1:16 ; Hé 11:3. YouVersion : ${link}`,
    ...
    S19:`Avec Actes 2 : Parole, Lumière, communauté ordonnée par l’Esprit (nouvelle création).`,
    ...
    S23:`Dire aux enfants : Dieu a tout fait avec amour ; nous sommes précieux ; prenons soin de la terre.`,
...
Suis exactement la correspondance:
s1=>"1. Prière d’ouverture", s2=>"2. Canon et testament", ..., s28=>"28. Prière de fin".
Pour "6. Structure littéraire", structure la section (ex. Genèse 1: "Jour 1…7").
...
Renvoie **uniquement** un JSON valide correspondant au schéma demandé.`.trim();
...
export default async function handler(req,res){
  try{
    ...
    // plus besoin de vérifier les titres: on construit nous-mêmes
    ...
    try{
      const url=new URL(req.url,`http://${req.headers.host}`); const p=parseQ(url.searchParams.get("q")||"");
      const md=fallbackMarkdown(p.book||"Genèse",p.chapter||1);
      res.setHeader("Content-Type","text/markdown; charset=utf-8"); res.setHeader("X-Last-Error",String(e?.message||e));
      return res.status(200).send(md);
    }catch{
      res.setHeader("Content-Type","text/markdown; charset=utf-8");
